...
const ListHarakat = props => {
  ...
  const choice = [
    ImagesPath.img.iqra.harakat.fathatain.normal,
    ImagesPath.img.iqra.harakat.kasratain.normal,
    ImagesPath.img.iqra.harakat.dammatain.normal
  ]
...
class Tanwin1 extends Component {
  ...
  twit = () => {
    ...
    let sound = SoundsPath.harakat[TANWIN_TYPE[harokat]][question.id]
  ...
  onTimeIsUp = () => {
    ...
    setTimeout(() => {
      ...
      this.setState({ star: star, result: true, help: false }, () => {
        this.props.setDataScore("tanwin1", { score, star })
        clearInterval(this.ticker)
      })
    }, 1000)
  }
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <ImageBackground
          style={{ flex: 1 }}
          source={ImagesPath.img.play.background.tanwin}
          resizeMode={"cover"}
        >
          <Header
            headerSource={ImagesPath.img.play.topBoard.guesstanwin}
            leftButton={{
        ...
        <HelpModal
          source={ImagesPath.img.play.helpImage.tanwin1}
          text={LANGUAGE.play.help.tanwin1}
          visible={help}
          close={() =>
