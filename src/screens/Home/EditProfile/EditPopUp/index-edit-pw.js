...
export default class LanguagePopUp extends Component {
      ...
      input: prevState.input.map((obj, index) =>
        id === index
          ? Object.assign(obj, {
              id: index,
              name: obj.name,
              value: this.removeEmojis(text.replace()),
              keyType: obj.keyType,
              secure: obj.secure,
              rule: obj.rule,
              error: obj.error
  ...
  textRequire(text, limit = 1) {
    return text.length >= limit
  }
  sameValue = (text1, text2) => {
    return text1 === text2
  }
  ...
  submitForm() {
    ...
    input.map((obj, i) => {
      for (let index = 0; index < obj.rule.length; index++) {
        ...
        let isValid = null
        if (rule === "sameValue") {
          isValid = this.sameValue(input[0].value, input[1].value)
          console.log("err")
        } else {
          isValid = this.textRequire(obj.value, param)
        }

        if (!isValid) {
          input[i].error = lang.auth.error[rule](
            lang.editProfile.textInput[obj.name].toLowerCase(),
            param
          )
          error = true
          this.setState({ isLoading: false, isSubmit: true })
          // console.log(input[i].error)
          break
        } else {
    ...
    if (!error) {
      ...
      data.append("password", input[0].value) // you can append anyone.
      data.append("confirm_password", input[1].value) // you can append anyone.
  ...
  render() {
    const { lang } = this.props
    const { disabled, isShowing, input, isSubmit, isLoading } = this.state
                      ...
                      >
                        <View style={{ flexDirection: "row" }}>
                          <Text style={styles[`inputText${lang.alfaType}`]}>
                            {lang.editProfile.textInput[item.name]}
                          </Text>
                          <View style={styles.inputStatus}>
                            <Animatable.Image
                              useNativeDriver={true}
                              animation={isSubmit ? "tada" : "bounceOut"}
                              source={
                                item.error
                                  ? ImagesPath.icon.wrong
                                  : ImagesPath.icon.check
                              }
                              style={styles.img}
                            />
                          </View>
                        ...
                        <View style={styles.inputBar}>
                          <TextInput
                            ...
                            returnKeyType={
                              index + 1 < input.length ? "next" : "send"
                            }
                            value={item.value}
                            underlineColorAndroid="rgba(0,0,0,0)"
                            onChangeText={text =>
                              this._changeText(text, item.id)
                            }
                            style={[
                              styles.input,
                        ...
                        <Animatable.Text
                          animation={isSubmit ? "bounceIn" : "bounceOut"}
                          useNativeDriver={true}
                    ...
                    <View style={{ width: "90%" }}>
                      ...
                      >
                        {isLoading ? (
                          ...
                          <Text
                            style={[
                              styles[`inputText${lang.alfaType}`],
                              {
                                color: "#fff",
